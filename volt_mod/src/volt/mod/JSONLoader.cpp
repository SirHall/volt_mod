...
ModuleJar JSONLoader::LoadModuleCollection(std::string const &collectionDir)
{
    ...
    for (auto &dir : std::filesystem::recursive_directory_iterator(collectionDir))
    {
        std::error_code err;
        if (dir.exists(err))
        {
            ...
            if (err)
            {
                throw std::runtime_error("Error occurred while trying to read file/directory: '" +
                                         dir.path().generic_string() + "': " + err.message());
                // Just incase we compile without exceptions
                return modules;
            }

            // Is this a directory
            if (dir.is_directory() && LoadModule(dir.path().generic_string(), modules))
            {
...
template <typename T>
std::vector<T> JSONGetVec(boost::property_tree::ptree const &root, std::string const &fieldName)
{
    ...
    if (child.has_value())
    {
        ...
        {
            auto childNodeVal = childNode.second.get_value_optional<std::string>();
...
bool JSONLoader::LoadModule(std::string const &moduleDir, std::vector<Module> &modules)
{
    ...
    {
        if (dir.is_regular_file() && dir.path().has_filename() && dir.path().filename() == "module.json")
        {
            ...
            try
            {
                boost::property_tree::read_json(dir.path().generic_string(), moduleFileRoot);
            }
            ...
            auto moduleName = moduleFileRoot.get_optional<std::string>("moduleName");
            ...
            // scripts)
            auto moduleDependencies = JSONGetVec<std::string>(moduleFileRoot, "dependencies");

            auto moduleLoadAfter = JSONGetVec<std::string>(moduleFileRoot, "loadAfter");
            ...
            // compatible with
            auto moduleTargetVersion = moduleFileRoot.get_optional<std::string>("targetVersion");

            auto moduleAuthor     = moduleFileRoot.get_optional<std::string>("author");
            auto moduleVersion    = moduleFileRoot.get_optional<std::string>("version");
            auto moduleSite       = moduleFileRoot.get_optional<std::string>("site");
            auto moduleRepository = moduleFileRoot.get_optional<std::string>("repository");
            ...
            modules.push_back(Module(jsonObjects, moduleName.value_or("NULL"), moduleDependencies, moduleLoadAfter,
                                     //    moduleLoadBefore,
                                     moduleVersion.value_or("0.0.0"), moduleTargetVersion.value_or("0.0.0"),
...
bool JSONLoader::RecursiveLoadDir(std::string const &loadDir, std::vector<JSONData> &jsonObjects)
{
    ...
    for (auto &dir : std::filesystem::recursive_directory_iterator(loadDir))
    {
        ...
        else if (dir.is_regular_file())
        {
            ...
            if (dir.path().has_filename() && dir.path().has_extension() && (dir.path().extension() == ".json") &&
                (dir.path().filename() != "module.json"))
            {
                ...
                boost::property_tree::read_json(dir.path().generic_string(), fileRoot);

                // Loaded the file, now find the mandatory labels

                // Firstly, is this json file supposed to be auto-loaded as
                // an asset?
                auto isJsonObjectAnAsset = fileRoot.get_optional<bool>("__asset_autoload__");
                ...
                auto jsonType = fileRoot.get_optional<std::string>("__asset_type__");
                if (!jsonType.has_value())
                {
                    throw std::runtime_error("Autoloaded JSON file has no '__asset_type__' field: " +
                                             dir.path().generic_string());
                    continue;
                }

                auto assetName = fileRoot.get_optional<std::string>("__asset_name__");
                if (!assetName.has_value())
                {
                    throw std::runtime_error("Autoloaded JSON file has no '__asset_name__' field: " +
                                             dir.path().generic_string());
                    continue;
                }

                // Now create the json object and push it onto the vector
                jsonObjects.push_back(JSONData(jsonType.value(), assetName.value(), fileRoot, dir.path()));
            }
        }
    }
