...
inline void MoveModuleToQueue(std::vector<Module> &modulesToLoad, std::vector<Module>::iterator it,
...
ModuleJar::ModuleJar(std::vector<Module> modulesToLoad)
{
    ...
    while (modulesToLoad.size() > 0)
    {
        ...
        {
            if (i->GetDependencies().size() == 0 && i->GetLoadAfter().size() == 0)
            {
            ...
            for (auto const &dep : i->GetDependencies())
            {
                if (std::none_of(this->modules.begin(), this->modules.end(),
                                 [&](Module const &loadedMod) { return loadedMod.GetModuleName() == dep; }))
                {
            ...
            if (allDepsLoaded)
            {
                ...
                for (auto const &loadAfter : i->GetLoadAfter())
                {
                    ...
                    if (std::any_of(modulesToLoad.begin(), modulesToLoad.end(), [&](Module const &existingMod) {
                            return existingMod.GetModuleName() == loadAfter;
                        }))
                    {
        ...
        if (modulesToLoad.size() == modulesToLoadCount)
        {
            ...
            throw std::runtime_error("Circular dependency, or missing dependency discovered");
        }
    }
}
...
ModuleJar &ModuleJar::operator=(const ModuleJar &other)
{
    ...
    modules.clear();
    std::copy(other.modules.begin(), other.modules.end(), std::back_inserter(modules));
    return *this;
}
